...
namespace engine
{
	void DissolutionInstances::init(std::shared_ptr<ShaderProgram> shader, std::shared_ptr<ShaderProgram> shaderDepthOnly)
	{
		this->shader = shader;
	...
	uint32_t DissolutionInstances::addModel(std::shared_ptr<Model> model, XMFLOAT3 size)
	{
	...
	void DissolutionInstances::addMaterials(std::vector<Material>& materials, uint32_t modelPos)
	{
	...
	void DissolutionInstances::addInstances(std::vector<Instance>& instances, uint32_t modelPos, uint32_t materialPos)
	{
		for (auto& mesh : perModel[modelPos].perMesh)
		{
	...
	void DissolutionInstances::render(Sky& sky)
	{
	...
	void DissolutionInstances::renderDepthOnly()
	{
	...
	void DissolutionInstances::clean()
	{
		shader = nullptr;
		for (auto& model : perModel)
		{
