...
namespace engine
{
	void DisappearanceInstances::init(std::shared_ptr<ShaderProgram> shader, std::shared_ptr<ShaderProgram> shaderDepthOnly)
	{
		this->shader = shader;
		vertexBufferID = shader->createUniform(sizeof(XMMATRIX), ShaderType::VERTEX);
		pixelBufferID = shader->createUniform(sizeof(Material::Properties) + sizeof(float) * 4, ShaderType::PIXEL);
	...
	void DisappearanceInstances::setNoiseTexture(std::shared_ptr<Texture> noiseTexture)
	{
		m_noiseTexture = noiseTexture;
	}

	void DisappearanceInstances::addModel(OpaqueInstances::PerModel& opaqueModel, uint32_t instanceID, uint32_t timePoint, Instance::Sphere& sphere)
	{
		bool instanceFound = false;
		for (auto& mesh : opaqueModel.perMesh)
		{
			for (auto& material : mesh.perMaterial)
			{
				material.material.noiseTexture = m_noiseTexture;
		...
		perModel.push_back({ opaqueModel, timePoint, sphere });
	}

	void DisappearanceInstances::addMaterials(std::vector<Material>& materials, uint32_t modelPos)
	{
	...
	void DisappearanceInstances::addInstances(std::vector<Instance>& instances, uint32_t modelPos, uint32_t materialPos)
	{
		for (auto& mesh : perModel[modelPos].perMesh)
		{
	...
	void DisappearanceInstances::updateInstanceBuffers()
	{
			...
			for (uint32_t meshIndex = 0; meshIndex < model.perMesh.size(); ++meshIndex)
			{
				...
				for (const auto& material : model.perMesh[meshIndex].perMaterial)
				{
					...
					for (uint32_t index = 0; index < numModelInstances; ++index)
					{
						...
						prop.startTimePoint = instances[index].startTimePoint;
						prop.sphere = instances[index].sphere;
						dst[copiedNum++] = prop;
					}
				}
			}
	...
	void DisappearanceInstances::render(Sky& sky)
	{
		...
		for (const auto& model : perModel)
		{
			...
			for (uint32_t meshIndex = 0; meshIndex < model.perMesh.size(); ++meshIndex)
			{
				...
				for (uint32_t materialIndex = 0; materialIndex < model.perMesh[meshIndex].perMaterial.size(); ++materialIndex)
				{
					...
					pbuff->spawnTime = DISSOLUTION_SPAWNTIME;
	...
	void DisappearanceInstances::spawnParticles(std::shared_ptr<ShaderProgram> shaderSpawner)
	{
		if (instanceBuffer.size() == 0)
			return;

		shaderSpawner->bind();
		s_devcon->PSSetShader(nullptr, NULL, NULL);
		...
		for (const auto& model : perModel)
		{
			...
			for (uint32_t meshIndex = 0; meshIndex < model.perMesh.size(); ++meshIndex)
			{
				...
				vertexCbuffer* vbuff;
				vbuff = (vertexCbuffer*)shaderSpawner->mapUniformVertex(0).pData;
				vbuff->mat = mesh.m_transform;
				shaderSpawner->unmapUniformVertex(0);
				shaderSpawner->bindBufferVertex(0, vertexBufferID);
	...
	void DisappearanceInstances::renderDepthOnly()
	{
	...
	void DisappearanceInstances::clean()
	{
		shader = nullptr;
	...
	DisappearanceInstances::Instance::Instance(OpaqueInstances::Instance& opaqInst, uint32_t timePoint, Instance::Sphere& sphere)
	{
		this->texCoordScale = opaqInst.texCoordScale;
		this->instanceID = opaqInst.instanceID;
		this->transformID = opaqInst.transformID;
		this->startTimePoint = timePoint;
		this->sphere = sphere;
	}

	DisappearanceInstances::PerMaterial::PerMaterial(OpaqueInstances::PerMaterial& opaqMat, uint32_t timePoint, Instance::Sphere& sphere)
	{
		this->material = opaqMat.material;
		for (auto& instance : opaqMat.instances)
		{
			this->instances.push_back({ instance, timePoint, sphere });
		}
	}

	DisappearanceInstances::PerMesh::PerMesh(OpaqueInstances::PerMesh& opaqMesh, uint32_t timePoint, Instance::Sphere& sphere)
	{
		this->transform = opaqMesh.transform;
		for (auto& material : opaqMesh.perMaterial)
		{
			this->perMaterial.push_back({ material, timePoint, sphere });
		}
	}

	DisappearanceInstances::PerModel::PerModel(OpaqueInstances::PerModel& opaqModel, uint32_t timePoint, Instance::Sphere& sphere)
	{
		this->model = opaqModel.model;
		for (auto& mesh : opaqModel.perMesh)
		{
			this->perMesh.push_back({ mesh, timePoint, sphere });
		}
	}
}
