...
namespace engine
{
	...
	inline const XMFLOAT3& getPos(const Mesh& mesh, uint32_t triangleIndex, uint32_t vertexIndex)
	{
	...
	void TriangleOctree::initialize(const Mesh& mesh)
	{
		...
		const XMFLOAT3 eps = { 1e-5f, 1e-5f, 1e-5f };
		m_box = m_initialBox = { mesh.box.min - eps, mesh.box.max + eps };

		for (uint32_t i = 0; i < mesh.triangles.size(); ++i)
		{
			const XMFLOAT3& V1 = getPos(mesh, i, 0);
			const XMFLOAT3& V2 = getPos(mesh, i, 1);
			const XMFLOAT3& V3 = getPos(mesh, i, 2);

			XMFLOAT3 P = (V1 + V2 + V3) / 3.f;
	...
	void TriangleOctree::initialize(const Mesh& mesh, const Box& parentBox, const XMFLOAT3& parentCenter, int octetIndex)
	{
		m_mesh = &mesh;
		m_children = nullptr;
		...
		m_box = m_initialBox;
		XMFLOAT3 elongation = (MAX_STRETCHING_RATIO - 1.f) * m_box.size();
	...
	bool TriangleOctree::addTriangle(uint32_t triangleIndex, const XMFLOAT3& V1, const XMFLOAT3& V2, const XMFLOAT3& V3, const XMFLOAT3& center)
	{
		...
		if (m_children == nullptr)
		{
			...
			{
				XMFLOAT3 C = (m_initialBox.min + m_initialBox.max) / 2.f;
				...
				{
					const XMFLOAT3& P1 = getPos(*m_mesh, index, 0);
					const XMFLOAT3& P2 = getPos(*m_mesh, index, 1);
					const XMFLOAT3& P3 = getPos(*m_mesh, index, 2);

					XMFLOAT3 P = (P1 + P2 + P3) / 3.f;
	...
	bool TriangleOctree::intersectSphere(const Ray& ray, MeshIntersection& nearest) const
	{
		return ray.intersectSphere(nearest, m_box);
	}
	...
	bool TriangleOctree::intersectInternal(const Ray& ray, MeshIntersection& nearest) const
	{
		...
		{
			const XMFLOAT3& V1 = getPos(*m_mesh, m_triangles[i], 0);
			const XMFLOAT3& V2 = getPos(*m_mesh, m_triangles[i], 1);
			const XMFLOAT3& V3 = getPos(*m_mesh, m_triangles[i], 2);
		...
		for (int i = 0; i < 8; ++i)
		{
			if ((*m_children)[i].m_box.contains(ray.r.origin))
			{
